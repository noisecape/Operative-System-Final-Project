    ...
    if (*head_ref == NULL){
        ...
        *head_ref = new_node;
        ...
        if((*head_ref)->execLeft == new_node->execLeft){
            orderHeadByID(head_ref, new_node);
        }else{
            new_node->next = *head_ref;
            *head_ref = new_node;
        }
...
void orderHeadByID(Task** head, Task* new_node){
...
void changeSchedulingPolicy() {
	...
	case Priority: 
		scheduler = ExecLeft;
		printf("Polita di scheduling: esecuzioni rimanenti\n\n");
		break;
	case ExecLeft:
		scheduler = Priority;
		printf("Polita di scheduling: priorita'\n\n");
		break;
	}
    sort();
}
